...
var (
  ...
  %[1]s resize-pod foo --requests=memory=1Gi,cpu=200m

  # Resize the limits of a pod named 'foo' to 2Gi of memory and 500m of CPU
  %[1]s resize-pod foo --limits=memory=2Gi,cpu=500m

  # Resize the requests of a container named 'bar' in a pod named 'foo' to 1Gi of memory and 200m of CPU
  %[1]s resize-pod foo bar --requests=memory=1Gi,cpu=200m`
)

// Command options
type ResizePodOptions struct {
...
func NewResizePodOptions(streams genericiooptions.IOStreams) *ResizePodOptions {
	return &ResizePodOptions{
...
func NewCmdResizePod(streams genericiooptions.IOStreams) *cobra.Command {
	o := NewResizePodOptions(streams)
	f := cmdutil.NewFactory(o.configFlags)

	cmd := &cobra.Command{
		Use:               "resize-pod [pod-name] [container] [flags]",
		Short:             "Resize the resources of a pod",
		Example:           fmt.Sprintf(resizePodExapmle, "kubectl"),
		SilenceUsage:      true,
		Version:           Version,
		ValidArgsFunction: argsPodNameAndContainerCompletionFunc(f),
		Annotations: map[string]string{
			cobra.CommandDisplayNameAnnotation: "kubectl resize-pod",
		},
...
func (o *ResizePodOptions) Complete(f cmdutil.Factory, cmd *cobra.Command, args []string) error {
	o.args = args
...
func (o *ResizePodOptions) Validate() error {
	var err error
...
func (o *ResizePodOptions) Run() error {
	allErrs := []error{}
